...
\section{Pogreške praćenja reference}
	...
	\begin{align}
		...
		&= d_1\Psi(\gamma(t), \eta(t)) \cdot (\gamma '(t) - \Ta(\gamma(t), \eta(t))\eta'(t)) \\
	...
	\begin{gather}
		e_R(R, R_d) = \frac{1}{2}(R_d^TR - R^TR_d)^{\text{V}} \\
		e_{\Omega}(R, \Omega, R_d, \Omega_d) = \Omega - R^TR_d\Omega_d
	\end{gather}
...
\chapter{Dodatak}
	...
	\begin{align*}
		...
		&= \frac{1}{2}(R_d^TR - R^TR_d)^\wedge \cdot (\Omega - R^TR_d\Omega_d) \\
		&= e_R \cdot e_\Omega
	\end{align*}
